...
class _CardCreate extends State<CardCreate> {
  ...
  Widget build(BuildContext context) {
    ...
    final _cardNumber = StreamBuilder(
  ...
  Widget cardColors(CardBloc bloc) {
    final dotSize =
        (MediaQuery.of(context).size.width - 220) / CardColor.baseColor.length;

    List<Widget> dotList = List<Widget>();

    for (var i = 0; i < CardColor.baseColor.length; i++) {
      dotList.add(StreamBuilder<List<CardColorModel>>(
            ...
            child: GestureDetector(
                ...
                child: Container(
                  ...
                  decoration: new BoxDecoration(
                      color: CardColor.baseColor[i], shape: BoxShape.circle),
                )),
          );
        },
      ));
